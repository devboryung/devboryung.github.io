...
Tomcat 다운로드 후, Eclipse의 Preferences 메뉴의 Server Runtime Environments항목을 통해 추가한다.<br>
<br>
...
수정 후 로딩이 완료되면 'Maven Dependencies'항목을 통해서 스프링 프레임워크 라이브러리들이 제대로 변경되었는지 확인한다.<br>

> Java version 변경

pom.xml 태그 중 maven-compiler-plugin의 내용을 1.6에서 1.8로 수정한다.<br>
그 후, 프로젝트를 우클릭 후 Maven -> Update Project를 진행한다.<br>


## Tomcat을 이용한 프로젝트 실행 확인

작성된 프로젝트가 정상적으로 동작하는지 Run As -> Run on Server를 이용해 프로젝트를 실행해서 확인한다.<br>
...
만약 lombok설치 후 바탕화면의 바로가기가 제대로 동작하지 않는다면 삭제 후 다시 생성한다.<br>
...
두 가지 모두 실습하기 위해 Java 설정을 이용하는 예제는 j를 붙여 프로젝트를 생성하겠다.<br>
...
pom.xml의 스프링 버전을 5.0.7로 수정하고, 컴파일 관련 버전도 1.8버전으로 수정한 후, Maven > Update Project를 실행한다.<br>
